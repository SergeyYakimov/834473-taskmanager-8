...
const getFilterElement = (title) => {
  const amount = getRandomInteger(FilterAmount.MIN, FilterAmount.MAX);
  return `
  <input
    type="radio"
    ...
    name="filter"
    ${title === filters[0] ? ` checked` : ``}
    ${amount === 0 ? ` disabled` : ``}
  >
  <label
    for="filter__${title.toLowerCase()}"
    class="filter__label">${title.toUpperCase()}
  <span
    class="filter__${title.toLowerCase()}-count">
      ${amount}
  </span>
  </label>
  `;
...
const getRandomCardColor = () => [`green`, `black`, `pink`, `yellow`, `blue`][getRandomInteger(0, 4)];
const getRandomCardModifier = () => [` card--repeat`, ` card--deadline`, ``][getRandomInteger(0, 2)];
const getRandomText = () => [`content`, `tasks`, `important`, `something`][getRandomInteger(0, 3)];

const taskParameters = {
  color: getRandomCardColor,
  classList: getRandomCardModifier,
  text: getRandomText,
  isDate: true,
  date: `24 February`,
  time: `22:00 PM`,
...
const getCardElement = (color, classList, text, isDate, date, time, isRepeat) => {
  return `
  <article class="card card--${color} ${classList}">
...
const onFiltersClick = () => {
  const taskCount = getRandomInteger(TasksCount.MIN, TasksCount.MAX);
  tasksContainer.innerHTML = ``;
  createTasks(taskCount, taskParameters);
};

const updateTasks = () => {
  const filtering = filtersContainer.querySelectorAll(`.filter__input`);
...
const createTasks = (count, task) => {
  for (let i = 0; i < count; i++) {
    tasksContainer.insertAdjacentHTML(`beforeend`,
        getCardElement(task.color(), task.classList(), task.text(), task.isDate, task.date, task.time, task.isRepeat));
  }
};

filters.forEach((item) => {
  filtersContainer.insertAdjacentHTML(`beforeend`, getFilterElement(item));
});

createTasks(TasksCount.MIN, taskParameters);
